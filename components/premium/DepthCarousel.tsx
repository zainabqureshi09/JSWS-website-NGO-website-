...
const DEFAULT_ITEMS: { image: string; alt: string }[] = [
...
const normalizeItem = (it: CarouselItem): { image: string; alt?: string } =>
...
const DepthCarousel = ({
  ...
  const layout = useCallback((pos: number) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const brightness = Math.max(
        0.15,
        1 - back * (cfg.falloff as number)
      );
      const blurPx =
        (cfg.blur as number) > 0
          ? Math.min(
              cfg.blur as number,
              (back / Math.max(1, cfg.visibleCards as number)) *
                (cfg.blur as number)
            )
          : 0;
      ...
      if (ov)
        ov.style.opacity = clamp(
          back * (cfg.falloff as number) * 1.25,
          0,
          0.86
        ).toFixed(3);
    }
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver((entries) => {
      ...
      const cfg = cfgRef.current as Record<string, number>;
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      autoTimerRef.current = setInterval(
        ...
        Math.max(
          (cfgRef.current as Record<string, number>).autoplayDelay,
          600
        )
      );
    };
